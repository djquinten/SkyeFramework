...
function keybindsClass:Add(CommandString, Category, Description, DefaultMapper, DefaultParameter, Event, OnPress, OnRelease, Once)
    RegisterCommand('+sumnicecommand'..CommandString, function()
        if IsPauseMenuActive() then return end
        if Once and RecentTriggers[CommandString] then return end

        if Event ~= '' then TriggerEvent(Event, true) end
        if OnPress then OnPress() end

        if Once then RecentTriggers[CommandString] = true end
    end, false)

    RegisterCommand('-sumnicecommand'..CommandString, function()
        if IsPauseMenuActive() then return end

        if Event ~= '' then TriggerEvent(Event, false) end
        if OnRelease then OnRelease() end

        if Once then RecentTriggers[CommandString] = nil end
    end, false)

    RegisterKeyMapping('+sumnicecommand'..CommandString, Category..' - ' .. Description .. '~', DefaultMapper, DefaultParameter)
end
